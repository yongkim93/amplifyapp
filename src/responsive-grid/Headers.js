...
const ColumnHeader = () => {
  const { state: windowSizeState } = useWindowSize();
  const { state: dateTimeState, dispatch: setDateTime } = useDateTimeManager();
  const elements = [];
  ...
  for (let i = 0; i < 7; i++) {
    elements.push(
      <div key={i} className="border-right border-bottom row-header background-color" style={style}>
